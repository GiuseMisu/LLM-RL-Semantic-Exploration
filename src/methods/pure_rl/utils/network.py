...
class BaseNet(nn.Module):
    ...
    def __init__(self, input_dim : int, output_dim : int = 1,  hidden_dim : int = 64, dropout : float = 0.2, device : torch.device = torch.device("cpu")):
        ...
        self.L3 = nn.Linear(hidden_dim, output_dim).to(device)
...
class MiniGridCNN(nn.Module):
    ...
    def __init__(self, output_dim: int = 128, device : torch.device = torch.device("cpu")):
        super().__init__()
        
        self.to(device)
        # Input: (batch, 3, 7, 7) - channels first for PyTorch
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=16, kernel_size=2, stride=1, device=device)
        self.conv2 = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=2, stride=1, device=device)
        self.conv3 = nn.Conv2d(in_channels=32, out_channels=64, kernel_size=2, stride=1, device=device)
        
        # Calculate flattened size: (7 -> 6 -> 5 -> 4) with kernel=2, stride=1
        self.fc = nn.Linear(64 * 4 * 4, output_dim, device=device)
        self.relu = nn.ReLU()
