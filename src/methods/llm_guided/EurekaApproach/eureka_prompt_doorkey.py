...
5. Object attributes (when env.grid.get(x,y) returns an object):
   - `.type`: string ('wall', 'door', 'key', 'goal', 'lava', etc.)
   - `.is_open`: bool (for doors - True if open)
   - `.is_locked`: bool (for doors - True if locked)
...
1. If Key Pickup is low (<20%), the agent isn't exploring enough or the reward for reaching the key is too weak.
